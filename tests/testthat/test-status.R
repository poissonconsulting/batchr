test_that("batch_file_status all processed successfully", {
  ...
  expect_identical(
    status,
    structure(list(type = "REMAING", time = structure(NA_real_, class = c(
      "hms",
      "difftime"
    ), units = "secs"), file = "file1.csv", message = NA_character_), row.names = c(
      NA,
      -1L
...
test_that("batch_file_status all failed processing", {
  ...
  expect_identical(
    status[c("type", "file", "message")],
    structure(list(type = "FAILURE", file = "file1.csv", message = "darn"), row.names = c(
      NA,
      -1L
    ), class = c("tbl_df", "tbl", "data.frame"))
  )
