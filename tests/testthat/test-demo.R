...
test_that("demo", {
  ...
  expect_identical(batch_config(fun, path = path, regexp = "file\\d[.]txt$"),
                   c("file2.txt", "file3.txt", "file4.txt"))
  
  config <- batch_config_read(path)
  
  expect_identical(names(config), 
  ...
  expect_identical(
    ...
    structure(list(type = c("SUCCESS", "FAILURE", "FAILURE"), 
                   file = c("file2.txt", "file3.txt", "file4.txt"), message = c(NA, 
                                                                                NA, "Uh, Houston, we've had a problem.")), class = c("tbl_df", 
                                                                                                                                     "tbl", "data.frame"), row.names = c(NA, -3L)))
  
  expect_identical(readLines(file.path(path, "file.txt")),
                   "the contents of file.txt")
  expect_identical(readLines(file.path(path, "file2.txt")),
                   "the modified contents of file2.txt")
  expect_identical(readLines(file.path(path, "file3.txt")),
                   "the contents of file3.txt")
  expect_identical(readLines(file.path(path, "file4.txt")),
