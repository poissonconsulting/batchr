...
test_that("batch_config update with existing recursive .batchr.rds files", {
  ...
  expect_error(batch_reconfig_fun(path, function(x) 1),
...
test_that("batch_reconfig_fileset update recurse with existing recursive .batchr.rds files", {
  ...
  expect_identical(batch_config(function(x) TRUE, path = path, recurse = FALSE,
                               regexp = "^file\\d[.]csv$"),
               "file2.csv")
  
  expect_identical(batch_files_remaining(path), "file2.csv")
  
  expect_error(batch_reconfig_fileset(path, recurse = TRUE),
               "^Subdirectories of '.*batchr' contain '[.]batchr[.]rds' files[.]$")
})
...
test_that("batch_reconfig_fileset update regexp", {
  ...
  expect_identical(batch_reconfig_fileset(path, regexp = "file\\d*[.]csv"),
                   c("file.csv", "file2.csv"))
  expect_identical(batch_reconfig_fileset(path, regexp = "file[.]csv"),
                   "file.csv")
  expect_identical(batch_reconfig_fileset(path, regexp = "filet[.]csv"),
                   character(0))
})
...
test_that("batch_reconfig_fileset update neither", {
  ...
  expect_error(batch_reconfig_fileset(path),
...
test_that("batch_reconfig_fileset update recurse", {
  ...
  expect_identical(batch_reconfig_fileset(path, recurse = TRUE),
                   c("batchr_sub/file3.csv", "file2.csv"))
  
  expect_identical(batch_reconfig_fileset(path, regexp = "file3[.]csv$"),
                   c("batchr_sub/file3.csv"))
  
  expect_identical(batch_reconfig_fileset(path, regexp = "_sub"),
                   character(0))
})
